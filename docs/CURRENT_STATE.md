...
- **Client experience:** the Client role can log in, view assigned projects/tasks/logs, and now see their payment requests (P3 M5 restricted shape) with budget figures correctly hidden (P3 M3 RBAC slice) — but this is not yet the PRD's full view-only photo portal.
